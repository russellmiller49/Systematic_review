...
export function StageQueue({
  ...
  const current =
    currentIndex >= 0 ? (data?.items[currentIndex] ?? null) : null;
  ...
  async function saveAbstract(event: React.FormEvent) {
    ...
    try {
      ...
      toast.error(
        error instanceof ApiError ? error.message : "Failed to add abstract",
      );
      if (
        error instanceof ApiError &&
  ...
  function submitDecision(
    ...
    const itemIndex =
      data?.items.findIndex((row) => row.citation.id === item.citation.id) ??
      -1;
    const nextSelection =
      ...
      if (filter === "UNDECIDED") {
        ...
        return {
          ...
          pagination: {
            ...
            totalPages: Math.max(
              1,
              Math.ceil(total / previous.pagination.limit),
            ),
          },
    ...
    apiPost(decisionsUrl, body)
      ...
      .catch((error) => {
        handledRef.current.delete(item.citation.id);
        toast.error(
          error instanceof ApiError ? error.message : "Failed to save decision",
        );
  ...
  async function confirmBatchExclude(exclusionReasonId: string) {
    ...
    try {
      ...
      toast.error(
        error instanceof ApiError
          ? error.message
          : "Failed to exclude selected articles",
      );
      setReloadKey((key) => key + 1);
    } finally {
      setBatchBusy(false);
  ...
  useScreeningShortcuts({
    blocked: excludeOpen || batchExcludeOpen || helpOpen,
      ...
      <div className="grid items-start gap-4 lg:grid-cols-[minmax(17rem,21rem)_minmax(0,1fr)]">
        ...
        >
          {data.pagination.total > 0 && (
            <>
              <ArticlePosition
                position={articlePosition || 1}
                total={data.pagination.total}
                saving={inFlight > 0}
                canNavigate={data.items.length > 1}
                onNavigate={navigateRelative}
                onHelp={() => setHelpOpen(true)}
              >
            ...
            <>
                  ...
                  abstractEditing ? (
                    ...
                    >
                      ...
                      <Textarea
                        ...
                        value={abstractDraft}
                        onChange={(event) =>
                          setAbstractDraft(event.target.value)
                        }
                        placeholder="Paste the article abstract here…"
                      />
                      <p className="text-xs text-muted-foreground">
                        This becomes shared, searchable citation metadata for
                        every screener.
                      </p>
                ...
                <div className="space-y-3">
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    <div className="flex flex-wrap items-center gap-1.5">
                      ...
                      {current.myDecision && (
                        <Badge
                          variant={DECISION_BADGE[current.myDecision.decision]}
                        >
                          Your decision:{" "}
                    ...
                    <span className="text-xs text-muted-foreground">
                      {current.completedReviews} of {current.requiredReviews}{" "}
                      required review
                    ...
                    <div className="rounded-md border border-border bg-muted/50 px-3 py-2 text-xs">
                      <div className="flex flex-wrap items-center gap-1.5">
                        ...
                        <Badge
                          variant={
                            DECISION_BADGE[
                              current.aiSuggestion.suggestedDecision
                            ]
                          }
                        >
                          suggests{" "}
                  ...
                  {current.canDecide ? (
                    <>
                      {current.myDecision && (
                        <p className="text-xs text-muted-foreground">
                          You can revise your decision until this citation
                          receives a final stage outcome.
                        </p>
                      )}
                      <DecisionControls
                        reasons={reasons}
                    ...
                    <div className="space-y-3">
                      <Alert>
                        ...
                        <strong>{current.finalOutcome?.toLowerCase()}</strong>{" "}
                        outcome. Screening decisions are locked unless an
                        authorized user reopens the stage result.
                      </Alert>
              ...
              {stage.blinded && (
                <p className="flex items-center gap-1.5 text-xs text-muted-foreground">
                  ...
                  Blinded screening — the article list shows review counts,
                  never a co-reviewer&apos;s choice.
                </p>
              )}
            </>
          ) : loading || inFlight > 0 ? (
